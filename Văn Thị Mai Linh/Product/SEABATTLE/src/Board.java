...
public class Board {
    ...
    // dat thuyen
    public void setShip(int []totalNum){
        ...
        if((xBegin != xEnd && yBegin != yEnd) || xEnd > rows || yEnd > cols) {
            System.out.println("Toạ độ thuyền không chính xác, vui lòng nhập lại.");
            System.out.println("Số lượng thuyền đã thêm " + totalNum[0]);
        }
        else {
            int ok = 0;
            ...
            for (int i = xBegin; i <= xEnd; i++) {
                for (int j = yBegin; j <= yEnd; j++) {
                    if(board[i][j] == 1){
                        ok = 1;
                        System.out.println("Đã có toạ độ thuyền bị trùng với thuyền khác, vui lòng nhập lại.");
                        System.out.println("Số lượng thuyền đã thêm " + totalNum[0]);
                        break;
                    }
    ...
    public void attackShip(Player player, Player enemy){
            ...
            if(check){
                ...
                player.increaseCellEnermyHited();
                System.out.println("Bạn đã bắn trượt, đến lượt đối thủ.Vui lòng nhập lựa chọn 5 để đến lượt đối thủ.");
                enemy.getBoard().displayFogBoard();
            }
